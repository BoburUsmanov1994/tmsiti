...

                    {
                        get(data, 'data.results', [])?.length == 0 ?
                            <div className={'col-span-12 mb-5 text-[#515D89] text-xl'}>Sizning so’rovingizga mos natija
                                ...
                                <Product template={(isActive == 0 || isActive == 2) ? 'list' : 'card'} viewUrl={get(split(get(material, 'resource_url'), '/'), '[1]')} img={'resource_image'} code={'resource_code'} name={'resource_name'} data={material}/>
