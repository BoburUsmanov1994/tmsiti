...
const Classifier = () => {
    ...
    const {data: chapters, isLoading: isLoadingChapters, isFetching: isFetchingChapters} = useGetQuery({
    ...
    const {data: groups, isLoading: isLoadingGroups, isFetching: isFetchingGroups} = useGetQuery({
                    ...
                    <div className={'col-span-12 '}>
                        <div className={'mb-5'}>
                            ...
                            <input onChange={debounce(function (e) {
                                   ...
                                   className={'border border-transparent w-full px-5 py-2.5  bg-white placeholder:italic placeholder:text-[#516164] h-[46px] rounded-[5px] outline-none focus:border-[#28366D]'}
                                   type="text"/>
                        ...
                        <ul className={'bg-white shadow-category p-2'}>
                            {
                                get(volumes, 'data.results', []).map((volume,i) => <li
                                    ...
                                    className={clsx('p-1.5 transition cursor-pointer mb-3 hover:bg-[#C7E3FC]', {'text-[#1B41C6] font-medium hover:bg-transparent': get(volume, 'id') == volumeId,'!mb-0':get(volumes, 'data.results', [])?.length == i+1})}
                                    key={get(volume, 'id')}>
                                    ...
                                    {get(volume, 'id') == volumeId && ((isLoadingParts || isFetchingParts) ?
                                        <ContentLoader classNames={'!bg-transparent min-h-[25vh]'}/> :
                                        <ul className={'pl-3 py-1.5'}>
                                            {get(parts, 'data.results', []).map((part, j) => <li onClick={(e) => {
                                                e.stopPropagation();
                                                setGroupId(null);
                                                setChapterId(null);
                                                ...
                                                {get(part, 'id') == partId && ((isLoadingChapters || isFetchingChapters) ?
                                                    <ContentLoader classNames={'!bg-transparent min-h-[25vh]'}/> :
                                                    <ul className={'pl-5 py-1.5'}>
                                                        {
                                                            ...
                                                            >
                                                                <div className={'flex items-start'}>
                                                                    <motion.div className={'mr-2 flex-none '} animate={{
                                                                        rotate: get(chapter, 'id') == chapterId ? 90 : 0,
                                                                    }}><Image width={18} height={18}
                                                                              src={get(chapter, 'id') == chapterId ? '/icons/arrow-down-category-active.svg' : '/icons/arrow-down-category.svg'}
                                                                              alt={'arrow'}/>
                                                                    </motion.div>
                                                                    <span>{get(chapter, 'chapter_name')}</span>
                                                                </div>
                                                                {get(chapter, 'id') == chapterId && ((isLoadingGroups || isFetchingGroups) ?
                                                                    <ContentLoader classNames={'!bg-transparent min-h-[25vh]'}/> : <ul className={'pl-9 py-1.5'}>
                                                                        {
                        ...
                        <div className="grid grid-cols-12 gap-x-8 ">
                            ...
                            <div className="col-span-12">
                                {volumeId ? <GridView
                                        ...
                                        params={search && search?.length > 3 ? {
                                            key: 'name',
                                            value: search
                                        } : {
                                            key: groupId ? 'group' : chapterId ? 'chapter' : partId ? 'part' : 'volume',
                                            value: groupId ? groupId : chapterId ? chapterId : partId ? partId : volumeId
                                        }}
                                        columns={columns}
                                    /> :
                                    <GridView
