  ...
  const dataExcel = [
    {
      ...
      company: "310037819",
      company_name:
        "QURILISHDA TEXNIK ME'YORLASH VA STANDARTLASHTIRISH ILMIY-TADQIQOT INSTITUTI DM",
      ...
      render: ({ row }) =>
        get(row, "order_status") === "new_order" ? (
          <div className={"flex flex-col gap-y-2"}>
            <button
              onClick={() => handleSendOrderStatus(get(row, "id"), "accepted")}
              className={
            ...
            <button
              onClick={() => handleSendOrderStatus(get(row, "id"), "canceled")}
              className={
          ...
          <div>
            <button
              onClick={() => handleSendOrderStatus(get(row, "id"), "sent")}
              className={
        ...
        ) : get(row, "order_status") === "customer_canceled" ? (
        ...
        ) : get(row, "order_status") === "customer_accepted" ? (
          <div className={"flex items-center gap-x-2  rounded-[6px]"}>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-transparent bg-opacity-30">
              <div className="bg-white p-8 rounded shadow-md w-[700px] h-[800px] overflow-y-scroll flex flex-col">
                <div className={"flex justify-between items-center mb-[30px]"}>
                  <button
                    className={"text-lg "}
                    onClick={() => handleListComment(isOpen)}
                  >
                    Ko'rish
                  </button>
                ...
                {extractedData?.map((item, index) => (
                  <div
                    key={index}
                    className={"border mb-[20px] shadow rounded-[10px]"}
                  >
                    ...
                    <div
                      className={"w-full h-[1px] bg-gray-400 my-[20px]"}
                    ></div>
                    ...
                    <div
                      ...
                      className={"p-2"}
                    >
                      {[...Array(get(item, "rating_company"))].map(
                        (star, index) => {
                          return (
                            <label
                              key={index}
