                ...
                <div className={" flex gap-x-[30px] mb-[50px]"}>
                    <div className={" min-w-[250px]"}>
                        <label htmlFor="dropzone-file"
                               className={'shadow-2xl ] rounded-[5px] cursor-pointer  flex flex-col justify-center items-center w-[240px] h-[170px] object-cover bg-white'}>
                            <Image src={'/icons/upload.svg'} alt={'upload'} width={48} height={48}/>
                            <p>yuklash</p>
                        </label>
                        <input id={"dropzone-file"} type={"file"} accept={"image/png, image/jpeg, image/jpg"} className={"hidden"}
                        />
                    </div>
                    ...
                    <div className={""}>
                        <form >
                            ...
                            {/*  Company description */}
                            {!description ?                             <p onClick={transformDescription} className={"text-xs text-black mb-[10px]"}>
                                ...
                                qiladi.
                            </p>  : <textarea name="description" id="" cols="100" rows="5" className={"p-[8px] text-xs text-black"}></textarea>}
                            {/*  Company manager  */}
                            {!manager ?                             <p onClick={transformManager} className={"text-xs text-black"}>
                                <strong>Rahbar: </strong>Melnikov Sergey Nikolayevich
                            </p> : <div className={"text-xs text-black flex items-center gap-x-[5px] my-[6px]"}>
                                <p onClick={transformManager} className={"cursor-pointer"}><strong>Rahbar: </strong></p>
                                <input type={"text"} placeholder={"Ism-familiyani kiriting"}  className={"p-[8px] min-w-[300px]"}/>
                            </div>}

                            {/*  Company phone  */}
                            {!phone ?                             <p onClick={transformPhone} className={`${phone ? "translate-y-1.5" : "translate-y-0"} transition-all duration-1000 text-xs text-black my-[6px] cursor-pointer`}>
                                <strong>Telefon: </strong>79-2236490
                            </p> : <div className={"text-xs text-black flex items-center gap-x-[5px] my-[6px]"}>
                                <p onClick={transformPhone} className={"cursor-pointer"}><strong>Telefon: </strong></p>
                                <input type={"tel"} placeholder={"Telefon raqamini kiriting"}  className={"p-[8px] "}/>
                            </div>}

                            {/*  Company address  */}
                            {!address ?                             <p onClick={transformAddress} className={"text-xs text-black"}>
                                <strong>Manzil: </strong>Navoiy viloyati, Navoiy sh., Navoiy
                                ko‘chasi, 3-uy
                            </p> : <div className={"text-xs text-black flex items-center gap-x-[5px] my-[6px]"}>
                                <p onClick={transformAddress}><strong>Manzil: </strong></p>
                                <input type={"text"} placeholder={"Manzilni kiriting"} className={"p-[8px] min-w-[500px]"}/>
                            </div>}
