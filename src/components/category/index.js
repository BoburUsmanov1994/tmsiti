...
const Category = ({data,url='materials/volume',name='volume_name',logo_url='volume_logo'}) => {
    return (
        <Link href={`/${url}/${get(data,'id')}`} className={'rounded-[5px] transition-all p-2.5 bg-white drop-shadow-category flex items-start min-h-full border border-transparent hover:border-[#017EFA]'}>
