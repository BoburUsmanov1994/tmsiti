...
const Product = ({template = 'card',data, name = 'material_name', code = 'material_csr_code', img = 'material_image',viewUrl='materials',classNames=''}) => {
    ...
    return (<>
            {
                template === 'list'  ? <Link href={`/${viewUrl}/${get(data, code)}`}>
                    <div
                ...
                </Link> : <div className={clsx('drop-shadow-category bg-white p-2.5 rounded-[5px] border border-transparent hover:border-[#017EFA] pb-12 min-h-full')}>
                            ...
                            <svg className={'fill-[#28366D] group-hover:fill-white '}  width={25} height={25} viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <defs>
                                    <clipPath id="clip0_1611_1155">
                                        <rect width={24} height={24} fill="white" transform="translate(0.5 0.883667)" />
                                    </clipPath>
                                </defs>
                            </svg>
                        </Button>
